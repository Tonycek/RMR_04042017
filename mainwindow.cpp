...
double cieloveX = 3450+500;//3450;//146;
double cieloveY = 1830+500;//138;
...
int MainWindow::demoCallback(CreateSensors inputData,void *ioPointer)
{
  ...
  //  currentWindow->update();
   ...
   //printf("nejde\n");
    ...
    if(fabs(ziadanyUhol-angle) > 10 && vzdialenostOdCiela > presnostZastavenia){
        mozemVpred=false;
    ...
    else if(fabs(ziadanyUhol-angle) > 7 && fabs(ziadanyUhol-angle) <= 10 && vzdialenostOdCiela > presnostZastavenia){
        mozemVpred=false;
